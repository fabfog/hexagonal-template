...
export default function registerFeatureCompositionWireHttpClientGenerator(plop: NodePlopAPI) {
  plop.setGenerator("feature-composition-wire-http-client", {
    description:
      "Wire a request-scoped HttpClient into a feature composition package `index.ts` (*InfrastructureProvider#getForContext) using @features/shared-infra-lib-http.",
    prompts: [
      {
        type: "list",
    ...
    actions: [
      (data?: Answers) => {
        ...
        const next = wireHttpClientIntoCompositionInfrastructure(indexPath, {
          propName,
        });
        fs.writeFileSync(indexPath, next, "utf8");
        return `Wired request-scoped HttpClient in ${path.relative(repoRoot, indexPath)}`;
      },
    ],
  });
}
